...
class SchemaPhpAlignmentTest {
    ...
    private function extractPhpTables() {
        ...
        $aliases = ['u', 's', 'ag', 'sga', 'be', 'ai', 'si', 'uc', 'tm', 'im', 'us', 'ur', 'cea', 'sea', 'st', 'n', 'r', 'p', 'c', 'sch', 'g', 'gsr', 'f', 'd', 'v', 'present_cte', 'absent_cte', 'alerts_cte', 'perm_cte', 'con', 'pg_stat_activity',
            'late_cte', 'existing', 'information_schema', 'detected_at', 'baseline', 'current', 'sub', 'intervals', 'i', 'rr', 'elm', 'ret', 'ai2', 'bev_stat', 'ev_stat', 's2',
    ...
    public function run() {
        ...
        $this->test("Seed: 10 roles (8 institucionales + GUARDIAN + SYSTEM_WORKER)", substr_count($this->sql, "uuid_generate_v4(), 'RECTOR'") === 1 && substr_count($this->sql, "uuid_generate_v4(), 'GUARDIAN'") === 1 && substr_count($this->sql, "uuid_generate_v4(), 'SYSTEM_WORKER'") === 1);
