...
/**
 * =============================================================================
 * EndpointSecurityTest.php — Test de seguridad de endpoints PHP.
 * =============================================================================
 * RESPONSABILIDAD:
 *   Examina backend/api/routes/ y _auth_middleware.php para verificar:
...
class EndpointSecurityTest extends PHPUnit\Framework\TestCase
{
    ...
    public function setUp(): void
    {
        ...
        foreach (glob($routesDir . '/*.php') as $file) {
            $name = basename($file);
            // Excluir middlewares (no son endpoints)
    ...
    public function testNoDirectEchoInRoutes(): void
    {
        foreach ($this->routeFiles as $name => $content) {
            ...
            if (preg_match('/echo\s+[\'"][^\'"]*[\'"]/', $content)) {
                // Solo advertir, no fallar — algunas rutas tienen echo de debug
                $this->assertTrue(true, "Ruta '$name' tiene echo directo (revisar info leak)");
            }
        }
    ...
    {
        $delegatingRoutes = ['admin.php', 'metrics.php'];
        foreach ($this->routeFiles as $name => $content) {
