    ...
    <div className="mx-auto max-w-3xl space-y-8">
        ...
        {Object.values(platforms).map((p) => (
          <button
            ...
            className={`rounded-surface border p-4 text-left transition-all duration-fast ${
              selected === p.id ? 'border-[var(--nx-accent)] bg-[color-mix(in_oklch,var(--nx-accent)_var(--nx-subtle-mix),transparent)]' : 'border-[var(--nx-border)] hover:border-[var(--nx-text-muted)]'
            }`}
          >
      ...
      <Card>
        <div className="flex items-start gap-4">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-control" style={{ backgroundColor: 'color-mix(in oklch, var(--nx-accent) var(--nx-subtle-mix), transparent)' }}>
