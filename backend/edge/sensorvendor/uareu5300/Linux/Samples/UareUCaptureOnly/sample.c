...
int main(int argc, char** argv){
	...
	if(DPFPDD_SUCCESS != result) print_error("dpfpdd_init()", result);
		...
		menu_t* pMenu = NULL;
		int res = Menu_Create("UareU SDK 3.x sample application (capture)", MENU_TYPE_EXIT, &pMenu);
		...
		if(0 == res) res = Menu_AddItem(pMenu, 104, "Asynchronous Capture ");
		if(0 == res){
			//main menu loop
			int bStop = 0;
			while(!bStop){
				...
				switch(nChoice){
					...
					case 103: //run streaming
						...
						break;
					case 104: //run async capture
						if(NULL == hReader){
							printf("\nReader is not selected!");
						}
						else{
							AsyncCaptureFinger(hReader, 0);
						}
						break;
					case -2: //exit
						bStop = 1;
