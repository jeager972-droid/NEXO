...
SELECT gen_random_uuid(),code,desc_text FROM (VALUES
    ('students.read','Ver estudiantes'),
    ('students.write','Crear/editar'),
    ('biometric.read','Ver biométricos'),
    ('reports.read','Ver reportes'),
    ('admin.full','Admin total'),
    ('twilio.send','Enviar mensajes'),
    ('devices.manage','Gestionar dispositivos')
) AS t(code,desc_text)
ON CONFLICT DO NOTHING;

-- =============================================================================
-- 3. USUARIOS ADMINISTRATIVOS (password: admin123 para todos)
-- Hash bcrypt: $2y$12$aHJm3fmaavJpHwmTC0fq1OyaBla2Nj.Rvxc9uWnPuYy.SFTXiu9My
-- =============================================================================

-- Admin RECTOR (rol máximo)
WITH admin_role AS (SELECT role_id FROM roles WHERE role_name='RECTOR' LIMIT 1)
...
WITH coord_role AS (SELECT role_id FROM roles WHERE role_name='COORDINATOR' LIMIT 1)
INSERT INTO users(user_id, school_id, role_id, document_number, first_name, last_name, email, phone, password_hash, password_salt, active, created_at)
...
WITH secretaria_role AS (SELECT role_id FROM roles WHERE role_name='SECRETARY' LIMIT 1)
INSERT INTO users(user_id, school_id, role_id, document_number, first_name, last_name, email, phone, password_hash, password_salt, active, created_at)
...
WITH portero_role AS (SELECT role_id FROM roles WHERE role_name='SECURITY' LIMIT 1)
INSERT INTO users(user_id, school_id, role_id, document_number, first_name, last_name, email, phone, password_hash, password_salt, active, created_at)
...
WITH auxiliar_role AS (SELECT role_id FROM roles WHERE role_name='AUXILIARY' LIMIT 1)
INSERT INTO users(user_id, school_id, role_id, document_number, first_name, last_name, email, phone, password_hash, password_salt, active, created_at)
...
FROM (VALUES
    (1,'222222222','Carlos','Martinez','cmartinez@colegionacional.edu.co'),
    (2,'333333333','Ana','Rodriguez','arodriguez@colegionacional.edu.co'),
    (3,'444444444','Luis','Gonzalez','lgonzalez@colegionacional.edu.co'),
    (4,'555555555','Maria','Lopez','mlopez@colegionacional.edu.co'),
    (5,'666666666','Pedro','Sanchez','psanchez@colegionacional.edu.co'),
    (6,'777777777','Jose','Ramirez','jramirez@colegionacional.edu.co'),
    (7,'888888888','Diana','Torres','dtorres@colegionacional.edu.co'),
    (8,'999999999','Camila','Castro','ccastro@colegionacional.edu.co')
) AS t(s,doc,fname,lname,email)
JOIN roles r ON r.role_name='TEACHER' ON CONFLICT(email) DO NOTHING;

-- =============================================================================
-- 4. AULAS Y GRUPOS ACADÉMICOS
-- =============================================================================
...
-- =============================================================================
-- 5. ESTUDIANTES (50 estudiantes generados dinámicamente)
-- =============================================================================
...
-- =============================================================================
-- =============================================================================
-- 6. ACUDIENTES (50 acudientes con user vinculado y rol GUARDIAN)
-- =============================================================================
...
BEGIN
    ...
    FOR i IN 1..50 LOOP
        ...
        INSERT INTO users(user_id,school_id,role_id,document_number,first_name,last_name,email,phone,password_hash,password_salt,active,created_at)
        VALUES(v_user,'a3333333-3333-3333-3333-333333333333'::UUID,v_guard_role_id,'g'||(100000000+i)::TEXT,gnames[1+(i%10)],'Acudiente '||i,'guardian'||i||'@test.com',CASE WHEN i=1 THEN '+573243607948' ELSE '+57'||(3000000000+i)::TEXT END,'$2y$12$aHJm3fmaavJpHwmTC0fq1OyaBla2Nj.Rvxc9uWnPuYy.SFTXiu9My','salt_guardian',TRUE,NOW())
        ON CONFLICT(email) DO NOTHING;
        IF FOUND THEN
            INSERT INTO guardians(guardian_id,user_id,whatsapp_phone,emergency_contact,created_at)
            VALUES(v_guard,v_user,CASE WHEN i=1 THEN '+573243607948' ELSE '+57'||(3000000000+i)::TEXT END,CASE WHEN i%5=0 THEN TRUE ELSE FALSE END,NOW())
...
JOIN (SELECT ROW_NUMBER() OVER() rn,student_id FROM students ORDER BY student_id) s ON g.rn=s.rn
ON CONFLICT DO NOTHING;
-- 7. DISPOSITIVOS EDGE
-- =============================================================================

INSERT INTO edge_devices(device_id,school_id,classroom_id,device_name,active,created_at) VALUES
    (gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,'c1aaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'::UUID,'Lector Entrada Principal',TRUE,NOW()),
    (gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,'c2bbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb'::UUID,'Lector Comedor',TRUE,NOW()),
    (gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,'c3cccccc-cccc-cccc-cccc-cccccccccccc'::UUID,'Lector Bloque B',TRUE,NOW())
ON CONFLICT DO NOTHING;

-- =============================================================================
-- 8. EVENTOS BIOMÉTRICOS (700 eventos distribuidos en mayo-agosto 2026)
-- =============================================================================

DO $$
DECLARE
    i INT;
    etypes TEXT[]:=ARRAY['INGRESO','SALIDA_ALMUERZO','REGRESO_ALMUERZO','SALIDA'];
    eresults TEXT[]:=ARRAY['SUCCESS','SUCCESS','SUCCESS','LATE','ABSENT'];
    sids UUID[];
    dids UUID[];
BEGIN
    SELECT array_agg(student_id::UUID) INTO sids FROM students WHERE school_id='a3333333-3333-3333-3333-333333333333'::UUID;
    SELECT array_agg(device_id::UUID) INTO dids FROM edge_devices WHERE school_id='a3333333-3333-3333-3333-333333333333'::UUID;

    -- 100 eventos del día actual
    FOR i IN 1..100 LOOP
    ...
    -- 200 eventos por mes (mayo, junio, julio, agosto)
    FOR i IN 1..200 LOOP
        INSERT INTO biometric_events(event_id,school_id,student_id,device_id,event_type,event_result,confidence_score,event_timestamp)
        VALUES(gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,sids[1+((i*7)%array_length(sids,1))],dids[1+((i*3)%array_length(dids,1))],etypes[1+((i+1)%4)],eresults[1+(i%5)],85.0+(random()*14.9),'2026-05-01'::DATE+((i%31)||' days')::INTERVAL+((7+(i%10))::TEXT||':'||(15+(i%45))::TEXT||':00')::TIME);
    END LOOP;
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,student_id,
    CASE WHEN random()<0.3 THEN 'LATE_ARRIVAL' WHEN random()<0.6 THEN 'EARLY_DEPARTURE' ELSE 'UNAUTHORIZED_ABSENCE' END,
    CURRENT_DATE+((8+(random()*6)::INT)::TEXT||':00:00')::TIME,
    CASE WHEN random()<0.5 THEN TRUE ELSE FALSE END,
    NOW()
FROM students WHERE school_id='a3333333-3333-3333-3333-333333333333'::UUID ORDER BY random() LIMIT 15;
...
FROM classrooms WHERE school_id='a3333333-3333-3333-3333-333333333333'::UUID ORDER BY random() LIMIT 5;
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,s.student_id,gsr.guardian_id,
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    'NOTIFICATION','OUTBOUND',g.whatsapp_phone,
    'Mensaje de prueba para '||s.first_name||' '||s.last_name,
    CASE WHEN random()<0.8 THEN 'SENT' ELSE 'FAILED' END,
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    CASE WHEN i%3=0 THEN 'STUDENT_UPDATE' WHEN i%3=1 THEN 'SEND_NOTIFICATION' ELSE 'GENERATE_REPORT' END,
    CASE WHEN i%3=0 THEN 'student' WHEN i%3=1 THEN 'guardian' ELSE 'report' END,
    gen_random_uuid(),
    jsonb_build_object('action','test','index',i),
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,
    ...
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    CASE WHEN random()<0.25 THEN 'UNAUTHORIZED_ACCESS' WHEN random()<0.5 THEN 'DATA_LEAK' WHEN random()<0.75 THEN 'PHYSICAL_THREAT' ELSE 'SUSPICIOUS_BEHAVIOR' END,
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    CASE WHEN random()<0.25 THEN 'LOGIN' WHEN random()<0.5 THEN 'LOGOUT' WHEN random()<0.75 THEN 'DATA_ACCESS' ELSE 'CONFIG_CHANGE' END,
    CASE WHEN random()<0.33 THEN 'user' WHEN random()<0.66 THEN 'student' ELSE 'system' END,
    gen_random_uuid(),
    jsonb_build_object('event','audit_test'),
    ('192.168.1.'||(10+(i%245))::TEXT)::INET,
    NOW()-(random()*INTERVAL'7 days')
FROM generate_series(1,30) i;

-- =============================================================================
-- 15. AUDIT DE REGISTROS DE ESTUDIANTE
-- =============================================================================
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,s.student_id,
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    CASE WHEN random()<0.5 THEN 'UPDATE' ELSE 'DELETE' END,
    jsonb_build_object('old','data'),
    jsonb_build_object('new','data'),
    NOW()-(random()*INTERVAL'30 days')
FROM students s WHERE s.school_id='a3333333-3333-3333-3333-333333333333'::UUID ORDER BY random() LIMIT 20;

-- =============================================================================
-- 16. REGISTROS DE PERSONAL
-- =============================================================================

INSERT INTO staff_records(staff_record_id,school_id,user_id,hired_at,position_name,active)
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,u.user_id,'2020-01-01','Docente de Aula',TRUE
FROM users u JOIN roles r ON r.role_id=u.role_id WHERE r.role_name='TEACHER' LIMIT 8;
...
SELECT gen_random_uuid(),ag.group_id,s.subject_id,c.classroom_id,
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    1+(i%5)::INT,i,
    ('08:00'::TIME+((i%6)::TEXT||' hours')::INTERVAL),
    ('09:30'::TIME+((i%6)::TEXT||' hours')::INTERVAL)
FROM academic_groups ag CROSS JOIN subjects s CROSS JOIN classrooms c, generate_series(1,15) i
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,s.student_id,
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    (SELECT schedule_id FROM schedules WHERE school_id='a3333333-3333-3333-3333-333333333333'::UUID ORDER BY random() LIMIT 1),
    'Salida anticipada',
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,s.student_id,
    ...
    'Salida pedagógica',
    CURRENT_DATE+CURRENT_TIME,
    CURRENT_DATE+CURRENT_TIME+INTERVAL'4 hours',
    'APPROVED'
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,s.student_id,
    ...
    'Museo de Bogotá',
    CURRENT_DATE+CURRENT_TIME,
    CURRENT_DATE+CURRENT_TIME+INTERVAL'6 hours',
    'Visita educativa'
FROM students s WHERE s.school_id='a3333333-3333-3333-3333-333333333333'::UUID ORDER BY random() LIMIT 5;
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    u.user_id,'Asunto '||i,'Mensaje de prueba número '||i,NOW()-(random()*INTERVAL'3 days')
FROM users u, generate_series(1,5) i
WHERE u.school_id='a3333333-3333-3333-3333-333333333333'::UUID LIMIT 15;
...
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,
    (SELECT user_id FROM users WHERE email='admin@nexo.edu' LIMIT 1),
    CASE WHEN i%3=0 THEN 'ATTENDANCE' WHEN i%3=1 THEN 'BEHAVIOR' ELSE 'BIOMETRIC' END,
    'PDF',
    NOW()-(random()*INTERVAL'7 days')
FROM generate_series(1,10) i;
...
FROM users u, generate_series(1,20) i
WHERE u.school_id='a3333333-3333-3333-3333-333333333333'::UUID LIMIT 20;
...
SELECT 'a3333333-3333-3333-3333-333333333333'::UUID,s.student_id,NOW()-(random()*INTERVAL'7 days'),
    ...
    CASE WHEN random()<0.25 THEN 'CRITICAL' WHEN random()<0.5 THEN 'HIGH' WHEN random()<0.75 THEN 'MEDIUM' ELSE 'LOW' END,
    30
