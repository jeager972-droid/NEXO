...
BEGIN
  ...
  FOR v_p IN SELECT permission_id FROM permissions p WHERE p.permission_code IN ('dashboard.global_view','students.create','students.view','consultations.global_view','reports.preview','reports.export','operations.solicitud','operations.situacion_critica') LOOP
    INSERT INTO role_permissions(role_permission_id,role_id,permission_id) VALUES(gen_random_uuid(),v_r,v_p) ON CONFLICT DO NOTHING;
  END LOOP;
  SELECT role_id INTO v_r FROM roles WHERE role_name='COUNSELOR' LIMIT 1;
  FOR v_p IN SELECT permission_id FROM permissions p WHERE p.permission_code IN ('dashboard.teacher_view','tracking.manage','behavior.view_risk','consultations.teacher_view','consultations.global_view','operations.seguimiento','operations.situacion_critica','students.view') LOOP
    INSERT INTO role_permissions(role_permission_id,role_id,permission_id) VALUES(gen_random_uuid(),v_r,v_p) ON CONFLICT DO NOTHING;
  END LOOP;
