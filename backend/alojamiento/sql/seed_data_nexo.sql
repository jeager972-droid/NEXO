...
INSERT INTO users(user_id, school_id, role_id, document_number, first_name, last_name, email, phone, password_hash, active, created_at)
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,role_id,'111111111','Administrador','NEXO','admin@nexo.edu','3000000000','$2y$12$aHJm3fmaavJpHwmTC0fq1OyaBla2Nj.Rvxc9uWnPuYy.SFTXiu9My',TRUE,NOW() FROM admin_role ON CONFLICT(email) DO NOTHING;

INSERT INTO users(user_id, school_id, role_id, document_number, first_name, last_name, email, phone, password_hash, active, created_at)
SELECT gen_random_uuid(),'a3333333-3333-3333-3333-333333333333'::UUID,r.role_id,doc,fname,lname,email,'300'||(1000000+s)::TEXT,'$2y$12$aHJm3fmaavJpHwmTC0fq1OyaBla2Nj.Rvxc9uWnPuYy.SFTXiu9My',TRUE,NOW()
...
INSERT INTO users(user_id,school_id,role_id,document_number,first_name,last_name,email,phone,password_hash,active,created_at)
VALUES(v_user,'a3333333-3333-3333-3333-333333333333'::UUID,v_guard_role_id,'g'||(100000000+i)::TEXT,gnames[1+(i%10)],'Acudiente '||i,'guardian'||i||'@test.com',CASE WHEN i=1 THEN '+573243607948' ELSE '+57'||(3000000000+i)::TEXT END,'$2y$12$aHJm3fmaavJpHwmTC0fq1OyaBla2Nj.Rvxc9uWnPuYy.SFTXiu9My',TRUE,NOW()) ON CONFLICT(email) DO NOTHING;
IF FOUND THEN
